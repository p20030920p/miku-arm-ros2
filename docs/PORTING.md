...
本文件记录本仓库从 catkin / ROS 1 Noetic 移植到 ament / ROS 2 Jazzy 时使用的
统一映射规则，以及每一处规则不适用的地方。

如果你要在此基础上继续改代码，请沿用这些约定，这样各个包的风格才能保持一致。

本文以中文书写，与源码注释语言一致。

## 目录

- **原始 ROS 1 源码（只读参考，禁止修改）**：`reference/ros1-original/`
- **ROS 2 移植版（所有改动写这里）**：`src/`
