...
// ROS 2 差异（OpenCV 版本差异）：OpenCV 4.6 的 <opencv2/imgproc/imgproc.hpp> 不再间接引入
// C 兼容常量头，threshold() 用到的 CV_THRESH_BINARY / CV_THRESH_OTSU 需要显式包含该头文件。
// 只是补一个头文件，检测算法与阈值行为完全不变（也可以用 cv::THRESH_BINARY | cv::THRESH_OTSU）。
