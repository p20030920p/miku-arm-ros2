...
> **ROS 2 Jazzy port.** This package was ported from ROS 1 (catkin/Noetic) to ROS 2 Jazzy
> (ament_cmake + rosidl). Build with `colcon build --packages-select aruco --symlink-install`
> and run with `ros2 launch aruco aruco.launch.py` (or `ros2 run aruco aruco`). The node name,
> parameters, topic names and message types are unchanged from the ROS 1 version: it publishes
> `/aruco/visible` (std_msgs/Bool), `/aruco/position` and `/aruco/rotation` (geometry_msgs/Point),
> `/aruco/pose` (geometry_msgs/PoseStamped), `/aruco/odom` (nav_msgs/Odometry) and subscribes
> `/camera/rgb` (sensor_msgs/Image), `/camera/info` (sensor_msgs/CameraInfo),
> `/marker_register` (aruco/msg/Marker) and `/marker_remove` (std_msgs/Int32).
> The text below is the original ROS 1 documentation.
