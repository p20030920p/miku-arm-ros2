...
#include <rclcpp/rclcpp.hpp>   // 用于 RCLCPP_WARN 打印
...
bool GravityCompensator::init(const KDL::Chain& chain)
{
    ...
    {
        RCLCPP_ERROR(arm_core_logging::logger(), "重力求解器创建失败");
        return false;
    }
...
void GravityCompensator::setGains(const std::vector<double>& g)
{
    ...
    {
        RCLCPP_WARN(arm_core_logging::logger(), "系数个数(%zu)与关节数(%u)不一致，忽略本次设置",
                 g.size(), tau_g.rows());
        return;
    }
...
void GravityCompensator::update(const KDL::JntArray& q)
{
    ...
    if (ret < 0)
    {
        RCLCPP_WARN(arm_core_logging::logger(), "重力矩计算失败（可能 URDF 惯量不完整）");
    }
}
