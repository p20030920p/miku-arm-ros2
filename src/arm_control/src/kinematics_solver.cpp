...
#include <rclcpp/rclcpp.hpp>  // 用于 RCLCPP_ERROR 打印
#include "arm_logger.h"
...
{
    // arm_core 库里的日志统一用固定名字 "arm_core"
    static rclcpp::Logger instance = rclcpp::get_logger("arm_core");
    return instance;
}
}  // namespace arm_core_logging
                                ...
                                const std::string& tip_link)
{
    ...
    {
        RCLCPP_ERROR(arm_core_logging::logger(), "无法读取 URDF 文件: %s", urdf_path.c_str());
        return false;
    }
    ...
    {
        RCLCPP_ERROR(arm_core_logging::logger(), "URDF 转 KDL 树失败");
        return false;
    }

    // 第 3 步：从树中取出 base_link → tip_link 这一段链
    // ROS 2 差异：orocos_kdl 1.5 的 Tree::getChain 返回 bool
    //             （ROS 1 时代返回 int，< 0 表示失败），故改为取反判断。
    if (!kdl_tree.getChain(base_link, tip_link, kdl_chain))
    {
        RCLCPP_ERROR(arm_core_logging::logger(), "找不到链: %s → %s", base_link.c_str(), tip_link.c_str());
        return false;
    }
                               ...
                               KDL::JntArray& sol)
{
    ...
    {
        RCLCPP_ERROR(arm_core_logging::logger(), "IK 求解器未初始化");
        return false;
    }
...
KDL::Frame KinematicsSolver::solveFK(const KDL::JntArray& q)
{
    ...
    {
        RCLCPP_ERROR(arm_core_logging::logger(), "FK 求解器未初始化");
        return result;
    }
    ...
    if (ret < 0)
    {
        RCLCPP_ERROR(arm_core_logging::logger(), "正运动学计算失败");
    }
