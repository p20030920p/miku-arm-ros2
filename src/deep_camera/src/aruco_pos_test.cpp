...
int main(int argc, char** argv)
{
    ...
    auto node = std::make_shared<rclcpp::Node>("aruco_pos_test");
    ...
    // 注意：原版只订阅了彩色图 + 两路红外（没有订阅深度图），此处保持一致。
    ...
    RCLCPP_INFO(getLogger(), "  ArUco 位姿识别工具已启动");
    RCLCPP_INFO(getLogger(), "  范围内标记显示变换矩阵，范围外仅显示 ID");
    ...
    while (rclcpp::ok())
    {
        ...
        if (g_frame_count % PRINT_EVERY_N_FRAMES == 0)
        {
            if (!markers.empty())
            {
                RCLCPP_INFO(getLogger(), "---- 第 %d 帧, 检测到 %zu 个标记 ----",
                ...
                for (size_t i = 0; i < markers.size(); i++)
                {
                    ...
                    if (!des.valid)
                    {
                        RCLCPP_INFO(getLogger(), "  [%zu] ID=%d (未在设计范围内)",
                                 i, markers[i].id);
                    }
                    else
                    {
                        ...
                        if (pr.valid)
                        {
                            RCLCPP_INFO(getLogger(), "  [%zu] ID=%d [%s] 距离=%.3f m "
                                     "RPY=(%.1f, %.1f, %.1f) deg",
    ...
    RCLCPP_INFO(getLogger(), "ArUco 位姿识别工具已关闭。");
