...
public:
    TrackNode() {
        ...
        sub = nh.subscribe<arm_control::ArmMsg>("Arm_rx", 20, &TrackNode::msgCallback, this);
    ...
    void run() {
        ...
        if (package_path.empty()) {
            ROS_FATAL("找不到功能包 'arm_control'。请检查工作区环境。");
            return;
        }
...
int main(int argc, char** argv) {
    ...
    // 启动多线程 ROS 回调
    ros::AsyncSpinner spinner(2); // 2个线程
    spinner.start();
