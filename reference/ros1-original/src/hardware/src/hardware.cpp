...
#include <ros/ros.h>
#include <serial/serial.h>  // ROS内置串口库头文件
...
void printHUD(const arm_control::ArmMsg& m, int gripper_protect,
...
private:
    ...
    // 自定义结构体消息
    arm_control::ArmMsg received_msg;
    arm_control::ArmMsg send_msg;
...
public:
    ...
    SerialNode() : nh() {
        // 初始化发布者订阅者
        pub = nh.advertise<arm_control::ArmMsg>("Arm_rx", 20);
        sub = nh.subscribe<arm_control::ArmMsg>("Arm_tx", 20, &SerialNode::msgCallback, this);
        ...
        while (ser.isOpen()!= true)
        {
            try {
                ...
                if (!error_reported) {
                    ROS_ERROR_STREAM("Unable to open port, retrying...");
                    error_reported = true;
                }
            }
        }
    ...
    void msgCallback(const arm_control::ArmMsg::ConstPtr& msg) {
        ...
        if (safe_pos_7 > GRIPPER_POS_UPPER_RAD) {
            ROS_WARN_THROTTLE(1.0, "[夹爪安全] 目标位置 %.2f° 超上限，已钳位至 %.2f°",
        ...
        if (safe_pos_7 < GRIPPER_POS_LOWER_RAD) {
            ROS_WARN_THROTTLE(1.0, "[夹爪安全] 目标位置 %.2f° 超下限，已钳位至 %.2f°",
